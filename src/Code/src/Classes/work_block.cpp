...
void WorkBlock::build (size_t st, const Work& W)
{
	...
	_T_score.setString (names::GetStringOfHyphenation (
			...
			(_RS_icon.getSize().x + WorksMenu::getIInBetween() + _RS_content.getSize().x + 2)
	...
	if (WorksMenu::getSizeBlockIcon ().y > _RS_content.getSize ().y)
	{
		_button.setSize (Vector2f (
				...
				WorksMenu::getBlockUp () + WorksMenu::getSizeBlockIcon ().y + WorksMenu::getBlockBetween ()
						+ _T_score.getGlobalBounds ().height / f + WorksMenu::getBlockUp ()
						));
	}
	else
	{
		_button.setSize (Vector2f (
				WindowAndStyles::getGlobalBounds ().width / f
						- WorksMenu::getIOutLeft () * 2 - 2 - WorksMenu::getIInLeft () * 2 - 2,
				WorksMenu::getBlockUp () + 1 + _RS_content.getSize ().y + 1 + WorksMenu::getBlockBetween ()
						+ _T_score.getGlobalBounds ().height / f + WorksMenu::getBlockUp ()
						));
	}
}
