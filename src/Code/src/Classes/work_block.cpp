...
void WorkBlock::build (size_t st, const Work& W)
{
	...
	T_content.setString (names::GetStringOfHyphenation (
			W.w_content,
			(WindowAndStyles::getGlobalBounds ().width / f
					- WorksMenu::getIOutLeft () * 2 - 2 - WorksMenu::getIInLeft () * 2 - 2
					- WorksMenu::getBlockLeft () * 2 - WorksMenu::getSizeBlockIcon ().x - WorksMenu::getBlockBetween () - 2 - 1)
					/ WorksMenu::getWidthSimvolMiddleSmallFont ()
					));
	...
	RS_content.setSize (Vector2f (
			WindowAndStyles::getGlobalBounds ().width / f
					- WorksMenu::getIOutLeft () * 2 - 2 - WorksMenu::getIInLeft () * 2 - 2
					- WorksMenu::getBlockLeft () * 2 - WorksMenu::getSizeBlockIcon ().x - WorksMenu::getBlockBetween () - 2,
	...
	T_score.setString (names::GetStringOfHyphenation (
			L"Score: " + to_wstring (W.test.f_best_score)
					+ L"\tBest true: " + to_wstring (W.test.f_best_true) + L"%"
					+ L"\tBest time: " + to_wstring (W.test.f_best_time) + L" c",
			(WorksMenu::getBlockUp () + 1 + WorksMenu::getSizeBlockIcon ().y + 1 + WorksMenu::getBlockBetween () - 2)
					/ WorksMenu::getWidthSimvolSmallFont ()
					));
	...
	if (WorksMenu::getSizeBlockIcon ().y > RS_content.getSize ().y)
	{
		button.setSize (Vector2f (
				WindowAndStyles::getGlobalBounds ().width / f
						- WorksMenu::getIOutLeft () * 2 - 2 - WorksMenu::getIInLeft () * 2 - 2,
				WorksMenu::getBlockUp () + WorksMenu::getSizeBlockIcon ().y + WorksMenu::getBlockBetween ()
						+ T_score.getGlobalBounds ().height / f + WorksMenu::getBlockUp ()
						));
	}
	else
	{
		button.setSize (Vector2f (
				WindowAndStyles::getGlobalBounds ().width / f
						- WorksMenu::getIOutLeft () * 2 - 2 - WorksMenu::getIInLeft () * 2 - 2,
				WorksMenu::getBlockUp () + 1 + RS_content.getSize ().y + 1 + WorksMenu::getBlockBetween ()
						+ T_score.getGlobalBounds ().height / f + WorksMenu::getBlockUp ()
						));
	}
}
...
void WorkBlock::resize (const Work& W)
{
	...
	T_score.setString (names::GetStringOfHyphenation (
			L"Score: " + to_wstring (W.test.f_best_score)
					+ L"     Best true: " + to_wstring (W.test.f_best_true) + L"%"
					+ L"     Best time: " + to_wstring (W.test.f_best_time * names::microsec) + L"c",
			(WorksMenu::getBlockUp () + 1 + WorksMenu::getSizeBlockIcon ().y + 1 + WorksMenu::getBlockBetween () - 2)
					/ WorksMenu::getWidthSimvolSmallFont ()
					));
	T_score.setCharacterSize (WorksMenu::getSmallFontScale () * f);
