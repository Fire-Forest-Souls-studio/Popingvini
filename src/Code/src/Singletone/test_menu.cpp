...
void TestMenu::main ()
{
	...
	if (names::game_status == GameStatus::test)
	{
		...
		TM_->m_RS_image.setPosition (
		    WorksMenu::getDownPanel ().getGlobalBounds ().left + WorksMenu::getDownPanel ().getGlobalBounds ().width - 1 * WindowAndStyles::getFactorY () - TM_->m_RS_image.getGlobalBounds ().width,
		    WorksMenu::getDownPanel ().getGlobalBounds ().top + WorksMenu::getDownPanel ().getGlobalBounds ().height - 1 * WindowAndStyles::getFactorY () - TM_->m_RS_image.getGlobalBounds ().height);
